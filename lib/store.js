 ...
 *
 * All persistence is backed by SQLite via Prisma (lib/db.js). The function
 * signatures mirror the original implementation so that the API routes only
 * need to add `await` — no structural changes required.
 *
 * serviceCategories is serialised as a JSON string in SQLite (which has no
 * native array type) and deserialised back to an array on every read.
 */
...
function toUser(row) {
  return {
    ...
    serviceCategories: row.serviceCategories
      ? JSON.parse(row.serviceCategories)
      : undefined,
...
export async function getLeadsForContractor(contractorId) {
...
export async function addLead(contractorId, lead) {
...
export async function updateLeadStatus(contractorId, leadId, status, notes) {
...
export async function getRequestsForHomeowner(homeownerId) {
...
export async function addRequest(homeownerId, data) {
...
export async function updateRequestStatus(homeownerId, requestId, status) {
