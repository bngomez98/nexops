...
export async function POST(req: Request) {
  const { role, requests, profile } = await req.json()

  if (role === 'homeowner') {
    // Summarize a homeowner's request history and surface maintenance advice
    const requestSummary = (requests ?? [])
      .slice(0, 10)
      .map((r: any) => `- [${r.status}] ${r.category}: ${r.title ?? r.description?.slice(0, 80)}`)
      .join('\n') || 'No requests yet.'
...
Be concrete and useful, not generic. If no history exists, give general Kansas homeowner spring maintenance advice.`,
  ...
  if (role === 'contractor') {
    ...
    const contractorInfo = profile
      ? `Trade category: ${profile.serviceCategories?.join(', ') || 'general'}, Rating: ${profile.averageRating ?? 'N/A'}, Active projects: ${profile.currentActiveProjects ?? 0}/${profile.maxActiveProjects ?? 3}`
