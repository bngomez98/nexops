                ...
                <div key={job.id as string} className="flex items-start gap-4 px-5 py-4">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-3 text-[12px] text-muted-foreground">
                      {!!job.properties && <span className="flex items-center gap-1"><MapPin className="w-3 h-3" />{(job.properties as Record<string, unknown>).city}, {(job.properties as Record<string, unknown>).state}</span>}
                      {!!job.client_profile && <span>Client: {(job.client_profile as Record<string, unknown>).full_name}</span>}
