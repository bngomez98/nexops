              ...
              {unmatchedJobs.map(job => (
                <div key={job.id as string} className="flex items-center gap-4 px-5 py-4">
                  <div className="flex-1 min-w-0">
                    ...
                    {!!job.properties && <p className="text-[12px] text-muted-foreground">{(job.properties as Record<string, unknown>).city as string}, {(job.properties as Record<string, unknown>).state as string}</p>}
            ...
            <div className="bg-card border border-border rounded-xl divide-y divide-border overflow-hidden">
              {matches.map((match: Record<string, unknown>) => (
                <div key={match.id as string} className="flex items-center gap-4 px-5 py-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="font-semibold text-[13.5px]">{match.jobs ? fmt((match.jobs as Record<string, unknown>).service_type as string) : 'Job'}</p>
